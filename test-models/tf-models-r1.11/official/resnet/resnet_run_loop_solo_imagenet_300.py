...
counter = 0
try:
  from official.utils.opt_esgd_solo_imagenet_imbalance import EagerSGDOptimizer
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  def my_func(x):
    if comm.rank == np.random.randint(comm.size) or comm.rank == np.random.randint(comm.size):
      time.sleep(0.32)
    return x
...
def resnet_main(
  ...
  if flags.FLAGS.horovod:
    ...
    if EagerSGDOptimizer is None:
      raise ImportError('Eager-SGD could not be imported. Deep500 is required')
...
def define_resnet_flags(resnet_size_choices=None):
  ...
  flags.DEFINE_bool(name='solodance', short_name='solo', default=False,
                    help='Use Deep500/Eager-SGD for distributed training')
